...
| Verse field = source §section for §-numbered extra-canonical texts (codifies SoT v5.31 change-log entry, 2026-05-11) | For texts whose received scholarly citation form is §section (Philo treatises, Josephus, certain pseudepigrapha and patristic works), the `verse` field carries the source §section number directly. Loeb / critical-edition citations land at URLs that match the citation — e.g., Conf. 146 → `/philo-conf/1/146`. The sequential-numbering validation check (§2) is waived for these books. Every chapter file declares the convention in `meta` and in the first verse's `translator_notes`. For multi-book treatises (e.g., Allegorical Interpretation = *Leg.* I–III; Special Laws = *Spec.* I–IV), `chapter` carries the Roman book number; `verse` carries the §section within that book. |
| Source-edition citation required on each verse for extra-canonical material (codifies `TCR_roadmap.md` §13) | Every verse in an extra-canonical book cites the critical edition it works from (Loeb volume + page, Cohn-Wendland, Niese, Charlesworth, etc.) in `translator_notes`. Raises the bar from "AI-generated rendering" to scholarly resource. |
| Reading-level target for patristic / scholarly material: 11th–12th grade (codifies `TCR_roadmap.md` §14) | Raised from the canonical-Bible 8th–10th-grade target. Applies to Philo, Josephus, pseudepigrapha, apostolic fathers, apologists, ante-Nicene fathers, patristic commentaries. |
