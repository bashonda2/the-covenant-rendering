...
|---|---|---|---|
| 1 | Dead Sea Scrolls | **PARTIAL** | **Isaiah** 66 ch / 590 variants at `/dss-isaiah/`; **Deuteronomy** 34 ch / 79 attestation entries / 4 theological variants (incl. 32:8 "sons of God", 32:43 longer ending → Heb 1:6) at `/dss-deuteronomy/`; **1 Samuel** 31 ch / 23 attestation entries / 7 variants (incl. the marquee Nahash paragraph at 11:1) at `/dss-1-samuel/`; **2 Samuel** 24 ch / 12 attestation entries / 2 variants (incl. Solomon's-gold-shields at 8:7) at `/dss-2-samuel/`; **Psalms** 151 ch (Pss 1–150 + Ps 151) / 15 attestation entries / 4 variants (incl. Ps 22:16 "they have pierced", Ps 145:13 nun-line restoration, Ps 151A/151B Hebrew Vorlage) at `/dss-psalms/`. Phase 4 remainder (~20 fragment-summary files for books with smaller attestation) outstanding. |
...
- DSS fragment viewer for partial-chapter rendering (still TODO; current `/dss-deuteronomy/` chapter pages render attestation entries directly, which works for fragmentary coverage but isn't a dedicated viewer).
- Phase 4 DSS expansion to Psalms (11QPsᵃ) and Samuel (4QSamᵃ).
