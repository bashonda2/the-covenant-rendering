...
18. **Qere/Ketiv notation correct.** If `text_hebrew` contains a qere/ketiv variant (where the written manuscript text differs from the traditional reading), verify the notation follows WLC/BHS convention: **Ketiv (written text) in square brackets `[...]`**, **Qere (read text) in parentheses `(...)`**. The Ketiv (square brackets) should appear first, followed by the Qere (parentheses). Example from Deut 28:30: `[ישגלנה] (יִשְׁכָּבֶ֔נָּה)`. Reversed notation misrepresents which variant is the manuscript reading and which is the traditional reading.
